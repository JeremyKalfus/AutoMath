...
/-- Vertices of `Γ(Z_5 × Z_5 × Z_3)` are the nonzero zero-divisors of the product ring, encoded by
the standard representatives `(a, b, c)` with `a, b ∈ {0, …, 4}` and `c ∈ {0, 1, 2}`. -/
abbrev Raw := Fin 5 × Fin 5 × Fin 3
...
def Adj (x y : Vertex) : Prop :=
  x ≠ y ∧
    ...
    ((cCoord x * cCoord y) % 3 = 0)

/-- `Label` is the machine encoding of the mathematical label set `{1, …, 42}` via `i ↦ i + 1`. -/
abbrev Label := Fin 42
...
theorem vertex_card : Fintype.card Vertex = 42 := by
  decide +kernel

theorem labelValue_bounds (ℓ : Label) : 1 ≤ labelValue ℓ ∧ labelValue ℓ ≤ 42 := by
...
theorem label_edgeCoprime : EdgeCoprime label := by
  intro x y hxy
  rcases hxy with ⟨hne, h5a, h5b, h3⟩
  have hne' : x.1 ≠ y.1 := fun h => hne (Subtype.ext h)
  exact raw_label_edgeCoprime x.1 y.1 x.2 y.2 hne' h5a h5b h3

theorem z5_z5_z3_zeroDivisorGraph_prime_checked : IsPrimeLabeling label := by
...
/-- Explicit natural-number form of the witness: the actual labels are the integers `{1, …, 42}`,
read from `Label` via `labelValue`. -/
theorem z5_z5_z3_zeroDivisorGraph_prime_explicit :
    ∃ f : Vertex → Label,
      Function.Bijective f ∧
      (∀ x, 1 ≤ labelValue (f x) ∧ labelValue (f x) ≤ 42) ∧
      (∀ x y, Adj x y → Nat.Coprime (labelValue (f x)) (labelValue (f y))) := by
  refine ⟨label, ?_, ?_, ?_⟩
  exact z5_z5_z3_zeroDivisorGraph_prime_checked.1
  intro x
  exact labelValue_bounds (label x)
  exact z5_z5_z3_zeroDivisorGraph_prime_checked.2

/-- The zero-divisor graph `Γ(Z_5 × Z_5 × Z_3)` admits a prime labeling. Here `Label = Fin 42` is
read as the mathematical label set `{1, …, 42}` via `labelValue i = i + 1`. -/
theorem z5_z5_z3_zeroDivisorGraph_prime : ∃ f : Vertex → Label, IsPrimeLabeling f := by
  exact ⟨label, z5_z5_z3_zeroDivisorGraph_prime_checked⟩
