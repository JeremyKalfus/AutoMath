...
A_labels = [
    ...
    27, 32, 36, 48, 54, 64, 72, 81, 96, 108,
]
B_labels = [1, 293, 307, 311]
C_labels = [
    n for n in range(2, 330)
    if math.gcd(n, 6) == 1 and n not in {293, 307, 311}
]

assert len(A_labels) == 20
assert len(B_labels) == 4
assert len(C_labels) == 106

used_labels = set(A_labels) | set(B_labels) | set(C_labels)
D_labels = [n for n in range(1, BOUND + 1) if n not in used_labels]
assert len(D_labels) == 424
...
assert set(labeling.values()) == set(range(1, BOUND + 1))
