...
/-- Standard representatives for elements of `Z_5 × Z_25`. -/
abbrev Raw := Fin 5 × Fin 25

instance : DecidableEq Raw := inferInstance

instance : Fintype Raw := inferInstance

/-- Vertices of `Γ(Z_5 × Z_25)` are the nonzero zero-divisors of the product ring, encoded by the
standard representatives `(a, b)` with `a ∈ {0, …, 4}` and `b ∈ {0, …, 24}`. -/
...
def Adj (x y : Vertex) : Prop :=
  x ≠ y ∧ ((aCoord x * aCoord y) % 5 = 0) ∧ ((bCoord x * bCoord y) % 25 = 0)

/-- `Label` is the machine encoding of the mathematical label set `{1, …, 44}` via `i ↦ i + 1`. -/
abbrev Label := Fin 44
...
theorem vertex_card : Fintype.card Vertex = 44 := by
  decide +kernel

theorem labelValue_bounds (ℓ : Label) : 1 ≤ labelValue ℓ ∧ labelValue ℓ ≤ 44 := by
...
theorem label_edgeCoprime : EdgeCoprime label := by
  intro x y hxy
  rcases hxy with ⟨hne, h5, h25⟩
  have hne' : x.1 ≠ y.1 := fun h => hne (Subtype.ext h)
  exact raw_label_edgeCoprime x.1 y.1 x.2 y.2 hne' h5 h25

theorem z5_z25_zeroDivisorGraph_prime_checked : IsPrimeLabeling label := by
...
/-- Explicit natural-number form of the witness: the actual labels are the integers `{1, …, 44}`,
read from `Label` via `labelValue`. -/
theorem z5_z25_zeroDivisorGraph_prime_explicit :
    ∃ f : Vertex → Label,
      Function.Bijective f ∧
      (∀ x, 1 ≤ labelValue (f x) ∧ labelValue (f x) ≤ 44) ∧
      (∀ x y, Adj x y → Nat.Coprime (labelValue (f x)) (labelValue (f y))) := by
  refine ⟨label, ?_, ?_, ?_⟩
  exact z5_z25_zeroDivisorGraph_prime_checked.1
  intro x
  exact labelValue_bounds (label x)
  exact z5_z25_zeroDivisorGraph_prime_checked.2

/-- The zero-divisor graph `Γ(Z_5 × Z_25)` admits a prime labeling. Here `Label = Fin 44` is read
as the mathematical label set `{1, …, 44}` via `labelValue i = i + 1`. -/
theorem z5_z25_zeroDivisorGraph_prime : ∃ f : Vertex → Label, IsPrimeLabeling f := by
  exact ⟨label, z5_z25_zeroDivisorGraph_prime_checked⟩
