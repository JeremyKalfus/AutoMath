...
## next_blocker
The exact `F2` slice is theorem-worthy and still looks non-rediscovered within the bounded audit, but it is still too thin to be paper-ready on its own. The `F25` ring-to-support bridge is already preserved, so the live blocker is now purely the four-class arithmetic wrapper on `Γ(Z_p × Z_25)`: the repo still needs either a quantified small-spill / barrier-reservoir theorem or a first sharp repaired-wrapper obstruction theorem, with `p = 19` the next live discriminator and `p = 23` next if `p = 19` survives.
